...
public interface ScanHistoryRepository extends JpaRepository<ScanHistory, Integer> {
    ...
    @Query(value = "SELECT user.id as id , first_name AS firstName, last_name AS lastName, username, unique_code AS uniqueCode, score, phone, duration, lat, lng FROM user JOIN " +
